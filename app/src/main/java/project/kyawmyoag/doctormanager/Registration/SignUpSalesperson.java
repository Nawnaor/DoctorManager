...
import project.kyawmyoag.doctormanager.Leaderboard.LeaderBoardObject;
...
public class SignUpSalesperson extends AppCompatActivity {
    ...
    void write_data(){
        ...
        if (TextUtils.isEmpty(email) || TextUtils.isEmpty(password) || TextUtils.isEmpty(name)
                ...
                Toast.makeText(getApplicationContext(), "အချက်အလက်အားလုံးမဖြည့်ရသေးပါ !!", Toast.LENGTH_SHORT).show();
                return;
        }

        if (password.length() < 6) {
            Toast.makeText(getApplicationContext(), "စကားဝှက်တိုလွန်းသည်, အနည်းဆုံး ၆ လုံးရိုက်ထည့်ပါ!", Toast.LENGTH_SHORT).show();
            return;
        }

        if(!isValidMobile(num)){
            Toast.makeText(getApplicationContext(), "ဖုန်းနံပါတ်မှားနေသည် !!", Toast.LENGTH_SHORT).show();
            return;
        }
        // email check
        if(!isValidEmail(email)){
            Toast.makeText(getApplicationContext(), "အီးမေးလ်မှားနေသည် !!", Toast.LENGTH_SHORT).show();
            return;
        }
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot dataSnapshot1 : dataSnapshot.getChildren()){
                    ...
                    if(sp.getEmailId().equals(email)){
                        Toast.makeText(getApplicationContext(),"အီးမေးလ်သည် မှတ်ပုံတင်ပြီးဖြစ်နေသည် !!", Toast.LENGTH_LONG).show();
                        tmp = -1;
                        break;
                    }
                    if(sp.getNumber().equals(num)){
                        Toast.makeText(getApplicationContext(),"ဖုန်းနံပါတ်သည် မှတ်ပုံတင်ပြီးဖြစ်နေသည် !!", Toast.LENGTH_LONG).show();
                        tmp = -1;
                        break;
                    }
                }
                ...
                databaseRef.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        if(x == 0){
                            Toast.makeText(getApplicationContext(),"ထိုမန်နေဂျာအမည်ဖြင့်မရှိပါ !!", Toast.LENGTH_LONG).show();
                            tmp = -1;
                        }
                        if(tmp != -1){
                                    ...
                                    .addOnCompleteListener(SignUpSalesperson.this, new OnCompleteListener<AuthResult>() {
                                        ...
                                        public void onComplete(@NonNull Task<AuthResult> task) {
                                            ...
                                            if (!task.isSuccessful()) {
                                                Toast.makeText(SignUpSalesperson.this, "မအောင်မြင်ပါ" + task.getException(),
                                                                        ...
                                                                        .addListenerForSingleValueEvent(new ValueEventListener() {
                                                                            ...
                                                                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                                                for(DataSnapshot snapshot1 : dataSnapshot.getChildren()){
                                                                                    ...
                                                                                    InventoryItem itNew = new InventoryItem(it.getItemName(),it.getItemName2(),it.getdate(),it.getTotal_available() - it.getSold(), 0, it.getProfit());
